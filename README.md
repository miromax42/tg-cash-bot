...
### GOHW-7
- [x] Добавить сервис по построению произвольных отчетов вашего бота.
```text
...
```
> Сервис реализован в `cmd/message-render/main.go`. Формат сообщения кафки `proto/kafka_report.proto`
- [x] Доп: добавить в интерцепторы gRPC метрики на вызываемые методы
> Server-side метрики добавлены на [эндпоинт](http://localhost:2112/metrics)
- [x] Доп: добавить метрики в продюсере ~~и консюмере~~
> Добавлена метрика на время записи и количество запросов
- [x] Доп: создать на стороне сервера grpc-gateway с возможностью вызова по gRPC и с помощью RESTful API
> Документация генерируется автоматически и доступна по [эндпоинту](http://localhost:8080/swagger/)
- [x] Доп: добавить валидацию запросов путем добавления плагина в proto файле
> валидация происходит через интерцептор
